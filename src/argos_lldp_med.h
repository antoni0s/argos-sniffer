...
/* Argos-Sniffer v6 staging engine: LLDP-MED.
 *
...
static inline void alm_text(const unsigned char *p, size_t n,
                            char *out, size_t cap) {
...
static inline const char *alm_class(uint8_t v) {
    switch (v) {
        case 1U: return "endpoint1";
        case 2U: return "endpoint2";
        case 3U: return "endpoint3";
        case 4U: return "network";
...
static inline const char *alm_app(uint8_t v) {
    switch (v) {
        case 1U: return "voice";
        case 2U: return "voice-signaling";
        case 3U: return "guest-voice";
        case 4U: return "guest-voice-signaling";
        case 5U: return "softphone-voice";
        case 6U: return "video-conferencing";
        case 7U: return "streaming-video";
        case 8U: return "video-signaling";
        default: return "reserved";
    }
}

static inline void alm_append(char *out, size_t cap,
                              const char *key, const char *value) {
    if (!out || !key || !value || !value[0] || cap == 0U) return;

    size_t used = strlen(out);
    if (used >= cap - 1U) return;
    (void)snprintf(out + used, cap - used, "%s%s=%s",
                   used ? ";" : "", key, value);
}

/* ANSI/TIA-1057 LLDP-MED organizational TLVs use OUI 00-12-BB.
 *
 * Location Identification (subtype 3) is deliberately ignored: Argos keeps
 * device/policy fingerprints and does not emit physical/civic location.
 * Extended Power via MDI (subtype 4) is intentionally left for a future
 * PoE-specific vector.
 */
static inline int argos_lldp_med_parse(const unsigned char *p, size_t n,
    ...
    while (pos + 2U <= n) {
        ...
        if (type == 127U && len >= 4U &&
            ...
            if (subtype == 1U && vlen >= 3U) {
                ...
                r->seen = 1;
                char *dst = subtype == 5U ? r->hardware :
                            subtype == 6U ? r->firmware :
                            subtype == 7U ? r->software :
                            subtype == 8U ? r->serial :
                            subtype == 9U ? r->manufacturer :
                            subtype == 10U ? r->model : r->asset;
    ...
    if (r->have_capabilities) {
        (void)snprintf(tmp, sizeof(tmp), "%s", alm_class(r->device_class));
        alm_append(r->detail, sizeof(r->detail), "class", tmp);
        (void)snprintf(tmp, sizeof(tmp), "0x%04x", (unsigned)r->capabilities);
        alm_append(r->detail, sizeof(r->detail), "caps", tmp);
    }

    if (r->have_policy) {
        (void)snprintf(tmp, sizeof(tmp), "%s,%s,%s,vlan=%u,prio=%u,dscp=%u",
                       alm_app(r->app_type),
                       r->policy_unknown ? "unknown" : "defined",
                       r->policy_tagged ? "tagged" : "untagged",
                       (unsigned)r->vlan, (unsigned)r->priority,
                       (unsigned)r->dscp);
