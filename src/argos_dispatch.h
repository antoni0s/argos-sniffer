...
#define ARGOS_VNC_PORT_FIRST 5900U
#define ARGOS_VNC_PORT_LAST 5999U
static inline int argos_vnc_port(uint16_t port) {
    return port >= ARGOS_VNC_PORT_FIRST && port <= ARGOS_VNC_PORT_LAST;
}
static inline int argos_dispatch_vnc_enabled(
    const argos_dispatch_plan_t *plan, uint16_t sport, uint16_t dport) {
    return argos_dispatch_protocol_enabled(plan, ARGOS_PROTOCOL_VNC) &&
