    ...
    while (running) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (protocol == IPPROTO_TCP) {
                ...
                if (telnet_tcp && payload_len > 0 &&
                    ae_telnet(buffer + payload_offset, payload_len, &ent_tcp) && ent_tcp.emit)
                    emit_enterprise_result(ARGOS_PROTOCOL_TELNET, &ent_tcp, src_mac,
                        src_ip_str, dst_ip_str, routed_str,
                        argos_dispatch_protocol_rate_limited(&dispatch_plan, ARGOS_PROTOCOL_TELNET));
