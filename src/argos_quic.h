...
static int argos_aes128_gcm_decrypt(const uint8_t key[16], const uint8_t iv[12], const uint8_t *aad, size_t aad_len, const uint8_t *ciphertext, size_t ciphertext_len, uint8_t *plaintext) {
    ...
    if (!argos_ct_equal16(tag, ciphertext + plain_len)) { if (plain_len != 0U) memset(plaintext, 0, plain_len); return 0; }
    return 1;
}
...
static int argos_quic_extract_clienthello(const uint8_t *frames, size_t frames_len, uint8_t *hello, size_t hello_cap, size_t *hello_len) {
    uint8_t *crypto = NULL, *present = NULL; size_t pos = 0U, crypto_cap = frames_len, contiguous = 0U; uint64_t i;
    if (hello == NULL || hello_len == NULL || hello_cap < 4U) return 0;
    *hello_len = 0U;
    ...
    crypto = (uint8_t *)calloc(1U, crypto_cap); present = (uint8_t *)calloc(1U, crypto_cap);
    if (crypto == NULL || present == NULL) { free(crypto); free(present); return 0; }
    ...
    while (pos < frames_len) {
        ...
        if (frame_type == 0x06U) { 
            ...
            if (length != 0U) { memcpy(crypto + (size_t)offset, frames + pos, (size_t)length); memset(present + (size_t)offset, 1, (size_t)length); }
            pos += (size_t)length; continue;
        }
        break; /* Stop cleanly when an unknown frame type is encountered. */
    }

    if (present[0] == 0U || crypto[0] != 0x01U) goto fail;
    if (present[1] == 0U || present[2] == 0U || present[3] == 0U) goto fail;
    {
        size_t body_len = ((size_t)crypto[1] << 16) | ((size_t)crypto[2] << 8) | (size_t)crypto[3];
        size_t total = 4U + body_len;
        if (total < 4U || total > crypto_cap || total > hello_cap) goto fail;
        for (i = 0U; i < (uint64_t)total; ++i) if (present[i] == 0U) goto fail;
        memcpy(hello, crypto, total); contiguous = total;
    }
    *hello_len = contiguous; free(crypto); free(present); return 1;

fail:
    free(crypto); free(present); return 0;
}
...
static inline int decrypt_quic_sni(const uint8_t *packet, int pkt_len, int dcid_pos, int dcid_len, uint8_t *fake_tls_buf, int fake_tls_buf_cap, int *fake_tls_len) {
    ...
    if (packet == NULL || fake_tls_buf == NULL || fake_tls_len == NULL) return 0;
    *fake_tls_len = 0;
    ...
    if (payload_len <= 16U) { free(aad); return 0; }
    ciphertext_len = payload_len;

    decrypted = (uint8_t *)malloc(payload_len - 16U);
    if (decrypted == NULL) { free(aad); return 0; }
    ...
    if (!argos_aes128_gcm_decrypt(quic_key, nonce, aad, aad_len, packet + payload_offset, ciphertext_len, decrypted)) {
        free(aad); free(decrypted); return 0;
    }
    free(aad);

    hello = (uint8_t *)malloc(ARGOS_QUIC_MAX_CRYPTO);
    if (hello == NULL) { free(decrypted); return 0; }
    if (!argos_quic_extract_clienthello(decrypted, payload_len - 16U, hello, ARGOS_QUIC_MAX_CRYPTO, &hello_len)) {
        free(hello); free(decrypted); return 0;
    }
    ...
    memcpy(fake_tls_buf + ARGOS_QUIC_FAKE_TLS_HEADER, hello, hello_len);
...
static inline void quic_heavy_gc(void) {
