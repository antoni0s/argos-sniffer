...
/* Argos-Sniffer v6 staging engine: STP family.
 *
 * Covers classic STP, RSTP and MSTP behind the future --stp vector.
...
static inline void amstp_digest_hex(char out[33], const unsigned char d[16]) {
    ...
    for (size_t i = 0U; i < 16U; ++i) {
        out[i * 2U] = hex[d[i] >> 4];
        out[i * 2U + 1U] = hex[d[i] & 0x0fU];
    }
    out[32] = '\0';
}

/* Classic IEEE 802.1D BPDU over 802.3 LLC:
...
static inline int argos_stp_parse(const unsigned char *p, size_t n,
    ...
    if (p[3] != 0x00U || p[4] != 0x00U) return 0;
    ...
    (void)snprintf(
        ...
        (unsigned)r->flags,
        (unsigned)r->root_priority,
        root,
        (unsigned)r->root_cost,
        (unsigned)r->bridge_priority,
        bridge,
        (unsigned)r->port_id,
        (unsigned)r->message_age,
        (unsigned)r->max_age,
        (unsigned)r->hello_time,
        (unsigned)r->forward_delay);

    return 1;
}

/* IEEE 802.1w RSTP: protocol version 2, BPDU type 0x02. */
static inline int argos_rstp_parse(const unsigned char *p, size_t n,
    ...
    if (p[0] != 0x42U || p[1] != 0x42U || p[2] != 0x03U) return 0;
    if (p[3] != 0x00U || p[4] != 0x00U ||
        p[5] != 0x02U || p[6] != 0x02U) return 0;
    ...
    (void)snprintf(
        ...
        (unsigned)((r->flags >> 4) & 1U),
        (unsigned)((r->flags >> 5) & 1U),
        (unsigned)((r->flags >> 6) & 1U),
        (unsigned)r->root_priority,
        root,
        (unsigned)r->root_cost,
        (unsigned)r->bridge_priority,
...
/* IEEE 802.1s / IEEE 802.1Q MSTP: version 3, BPDU type 0x02.
 *
 * Privacy rule: the raw 32-byte MST Configuration Name is parsed only by
 * position and is never copied or emitted. Revision and digest are retained
 * because they identify region configuration without exposing a site label.
 * Only the first MSTI message is summarized; msti_count records the total.
 */
static inline int argos_mstp_parse(const unsigned char *p, size_t n,
    ...
    amstp_digest_hex(digest, r->config_digest);
    astp_mac(cist_bridge, r->cist_bridge_mac);
    if (r->msti_count > 0U) {
        astp_mac(msti_root, r->first_msti_root_mac);
    } else {
        (void)strcpy(msti_root, "none");
    }
    ...
    (void)snprintf(
        ...
        (unsigned)r->version3_length,
        (unsigned)r->config_revision,
        digest,
        (unsigned)r->cist_internal_root_cost,
        (unsigned)r->cist_bridge_priority,
        cist_bridge,
        (unsigned)r->cist_remaining_hops,
        (unsigned)r->msti_count,
        (unsigned)r->first_msti_flags,
        (unsigned)r->first_msti_root_priority,
        msti_root,
        (unsigned)r->first_msti_root_cost,
        (unsigned)r->first_msti_bridge_priority,
        (unsigned)r->first_msti_port_priority,
        (unsigned)r->first_msti_remaining_hops);
