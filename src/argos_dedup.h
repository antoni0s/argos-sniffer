...
/* Explicit lifecycle work only, before packet processing. Repeated prepare
 * preserves entries; failure leaves the owner empty and permits explicit retry. */
static inline int argos_dedup_prepare(argos_dedup_state_t *state) {
    if (!state) return 0;
    if (!state->table)
...
/* Deterministic allocation-free core. An unprepared/failed cache is fail-open:
 * telemetry is never suppressed and packets never retry preparation.
 * Keep one lookup body: removing allocation otherwise encourages duplicated
 * inlining into emitters. Existing native callers already use a shared call. */
#if defined(__GNUC__) || defined(__clang__)
__attribute__((noinline, unused))
#endif
static int argos_dedup_should_suppress_at(argos_dedup_state_t *state,
                                                 const char *mac,
                                                 const char *evtype,
                                                 const char *payload,
