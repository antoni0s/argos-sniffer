...
static void test_hello_retry_request(void)
{
    ...
    argos_tls_server_staging_result_t out;
    assert(sizeof(record) == 5U + 0x36U);
    assert(argos_tls_server_staging_parse(record, sizeof(record), &out) == 1);
    assert(out.is_hello_retry_request == 1U);
    assert(out.negotiated_version == 0x0304U);
    for (size_t n = 0; n < sizeof(record); ++n)
        assert(argos_tls_server_staging_parse(record, n, &out) == 0);
}
...
static void test_alpn_and_session_flags(void)
{
    ...
    argos_tls_server_staging_result_t out;
    /* Synthetic extension-presence coverage only. This does not prove that
     * TLS 1.3 ALPN/early_data/ECH acceptance is visible in ServerHello. */
    assert(sizeof(record) == 5U + 0x49U);
    assert(argos_tls_server_staging_parse(record, sizeof(record), &out) == 1);
